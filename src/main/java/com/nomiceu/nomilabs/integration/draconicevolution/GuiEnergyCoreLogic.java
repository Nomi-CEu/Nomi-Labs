...
public class GuiEnergyCoreLogic {
    ...
    public static void drawBackground(GuiEnergyCore gui, FontRenderer fontRenderer) {
        ...
        String stabText = translate("gui.de.stabilizers.txt") + ": " +
                (gui.tile.stabilizersOK.value ? translate("gui.de.valid.txt") : translate("gui.de.invalid.txt"));
        GuiHelper.drawCenteredString(fontRenderer, stabText, gui.guiLeft + gui.xSize / 2, gui.guiTop + 18, stabColour,
                gui.tile.stabilizersOK.value);
        if (gui.tile.tier.value >= 5) {
            GuiHelper.drawCenteredString(fontRenderer, translate("gui.de.advancedStabilizersRequired.txt"),
                    gui.guiLeft + gui.xSize / 2, gui.guiTop + 28, 0x777777, false);
        }

        int coreColour = gui.tile.coreValid.value ? 0x00FF00 : 0xFF0000;
        String coreText = translate("gui.de.core.txt") + ": " +
                (gui.tile.coreValid.value ? translate("gui.de.valid.txt") : translate("gui.de.invalid.txt"));
