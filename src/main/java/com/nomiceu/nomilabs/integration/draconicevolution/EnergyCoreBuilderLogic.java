...
public class EnergyCoreBuilderLogic {
     ...
     * Returns isDead.
     */
    public static boolean build(TileEnergyStorageCore core, EntityPlayer player, Map<BlockPos, BlockStates> workList,
                                LinkedList<BlockPos> workOrder) {
        workList.clear();
        workOrder.clear();
     ...
     * Returns isDead.
     */
    private static boolean buildWorklist(TileEnergyStorageCore core, EntityPlayer player,
                                         Map<BlockPos, BlockStates> workList, LinkedList<BlockPos> workOrder) {
    ...
    private static void buildInstant(World world, EntityPlayer player, Map<BlockPos, BlockStates> workList,
                                     LinkedList<BlockPos> workOrder) {
     ...
     * Returns isDead.
     */
    public static boolean updateBuildProcess(World world, EntityPlayer player, Map<BlockPos, BlockStates> workList,
                                             LinkedList<BlockPos> workOrder) {
     ...
     */
    private static boolean updateProcess(World world, EntityPlayer player, Map<BlockPos, BlockStates> workList,
                                         LinkedList<BlockPos> workOrder) {
        ...
        if (!world.isAirBlock(pos)) {
            ...
            player.sendMessage(new TextComponentTranslation("ecore.de.assemble_error_expected_air.txt", pos.toString())
                    .setStyle(new Style().setColor(TextFormatting.RED)));
            return true;
        }
        ...
        if (player.capabilities.isCreativeMode || DraconicHelpers.extractItemOfBlockStates(states, player)) {
            ...
            player.sendMessage(new TextComponentTranslation("ecore.de.assemble_missing_required_new.txt", required)
                    .setStyle(new Style().setColor(TextFormatting.RED)));
            return true;
        }
        return false;
